...
if sys.version > '3.':
    sys.exit(sys.stderr.write('Please run uploader_goapp.py by python2\n'))

os.chdir(os.path.abspath(os.path.dirname(__file__)))
GAE_DIR = os.getenv('GAE_DIR') or 'gae'
CACHE_DIR = 'cache'
GOAPP = os.path.dirname(os.path.abspath("__file__")) + "/go_appengine/goapp"
PROCESSES = 5
RETRYTIMES = 3
...
patch_google_appengine_sdk('./go_appengine',
                           {
...
sys.path = ['go_appengine'] + sys.path
import httplib2
...
def upload_cache_dir(dirname, appid):
...
def retry_upload(max_retries, dirname, appid):
    dirname = upload_cache_dir(dirname, appid)
...
===============================================================
 ...
 Linux/Mac 用户, 请使用 python uploader_goapp.py 来上传应用
===============================================================
...
特别提醒：appid 请勿包含 ID/Email 等个人信息！
    ...
    if not os.path.isdir(CACHE_DIR):
        os.mkdir(CACHE_DIR)
    appids = input_appids()
    retry_upload(RETRYTIMES, GAE_DIR, appids[0])
    pool = multiprocessing.pool.ThreadPool(processes=PROCESSES)
    pool.map(functools.partial(retry_upload, RETRYTIMES, GAE_DIR), appids[1:])
