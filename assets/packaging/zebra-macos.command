...
class ZebraHelpers(object):
    ...
    def check_update(self):
        script = '''tell application "Terminal"\nactivate\ndo script "%s/get-latest-zebra.sh"\nend tell''' % os.getcwd()
        result, error = NSAppleScript.alloc().initWithSource_(script).executeAndReturnError_(None)
        return result, error
    ...
    def import_rootca(self, certfile):
        ...
        cmds.append('security delete-certificate -c Zebra')
...
class ZebraMacOS(NSObject):

    console_color = ColorSet[0]

    def applicationDidFinishLaunching_(self, notification):
        self.helper = ZebraHelpers()
        self.setupUI()
        self.startZebra()
        self.notify()
    ...
    def setupUI(self):
        ...
        # Set initial image
        raw_data = base64.b64decode(''.join(ZEBRA_ICON_DATA.strip().splitlines()))
        ...
        # Set a tooltip
        self.statusitem.setToolTip_(ZEBRA_TITLE)
        ...
        self.console_window.setTitle_(ZEBRA_TITLE)
    ...
    def startZebra(self):
        cmd = '%s/zebra' % os.path.dirname(os.path.abspath(__file__))
    ...
    def notify(self):
        ...
        notification.setTitle_("Zebra macOS Started.")
...
def precheck():
    ...
    if not has_user_json:
        ...
        alert.setMessageText_('Zebra Information:')
        alert.setInformativeText_('Please configure your zebra at first.\nFor example, add a new gae.user.json')
...
def main():
    ...
    delegate = ZebraMacOS.alloc().init()
