...
if platform.mac_ver()[0] > '10.':
    sys.exit(os.system(
        'osascript -e \'display dialog "Please use zebra-macos rather than zebra-gtk" buttons {"OK"} default button 1 with icon caution with title "Zebra GTK"\''))
...
try:
    ...
    sys.exit(os.system(
        'gdialog --title "Zebra GTK" --msgbox "Please install python-gtk2" 15 60'))
try:
    import pynotify
    pynotify.init('Zebra Notify')
except ImportError:
    pynotify = None
try:
    import appindicator
...
def rewrite_desktop(filename):
    ...
    with open(filename, 'wb') as fp:
        content = re.sub(
            r'(?m)Exec=.*', '''Exec=bash -c "export PATH=$PATH:`dirname '%%k'`:%s; exec zebra-gtk.py"''' % os.getcwd(), content)
        content = re.sub(r'Icon=\S*', 'Icon=%s/zebra-gtk.png' %
                         os.getcwd(), content)
        fp.write(content)
...
class ZebraGTK:
    ...
    command = [
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'zebra'), '-v=2']
    message = u'Zebra already started.'
    fail_message = u'Zebra start failed, see the terminal'
    ...
    def __init__(self, window, terminal):
        ...
        logo_filename = os.path.join(
            os.path.abspath(os.path.dirname(__file__)), 'zebra-gtk.png')
        self.window.set_icon_from_file(logo_filename)

        if appindicator:
            self.trayicon = appindicator.Indicator(
                'Zebra', 'indicator-messages', appindicator.CATEGORY_APPLICATION_STATUS)
            ...
            self.trayicon.set_tooltip('Zebra')
    ...
    def show_notify(self, message=None, timeout=None):
        if pynotify and message:
            notification = pynotify.Notification('Zebra Notify', message)
...
def main():
    ...
    if os.path.isfile('zebra-gtk.desktop'):
        rewrite_desktop('zebra-gtk.desktop')

    ZebraGTK(gtk.Window(), vte.Terminal())
    gtk.main()
